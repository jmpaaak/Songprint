module.exports = function (default_data) {
    ...
    var diffSumUpperBound = 2000; // 에지를 이을 상한값
    var stage = 300; // 단계
    var unitEdgeLength = 1 / stage; // 최대길이 / stage
    var edgeLengthArr = [];
    var diffSumUpperRangeArr = [];
    for(var i=0; i < stage; i++) {
      edgeLengthArr[stage-1-i] = (i+1) * unitEdgeLength; // 단계 별로 에지 길이 배분
      // console.log(edgeLengthArr[i].toFixed(1));
      edgeLengthArr[stage-1-i] = parseFloat(edgeLengthArr[stage-1-i].toFixed(5)); // 소수점 여섯째 자리에서 버림
