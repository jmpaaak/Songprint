module.exports = function (default_data) {
    ...
    // set tone data
    for(var i=6; i<lines.length; i++) { // after 6 is tone data
    ...
    for(var i=0; i < songNames.length; i++) {
        ...
        // 소스와 데스티네이션의 순서가 연도의 흐름과 맞는지 확인
          var sRaw = releaseYearList[edgeNew.source];
          var tRaw = releaseYearList[edgeNew.target];

          var sChars = sRaw.split("");
          var tChars = tRaw.split("");
          ...
          var sStr = sChars.join("");
          var tStr = tChars.join("");

          var sDate = new Date(sStr);
          var tDate = new Date(tStr);
          ...
          if(sDate < tDate)
              ...
              var temp = edgeNew.source;
              edgeNew.source = edgeNew.target;
              edgeNew.target = temp;
              JSONGraphData["links"].push(edgeNew);
          }
    }
